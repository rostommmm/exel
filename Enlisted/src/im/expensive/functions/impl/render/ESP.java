        ...
        for (Map.Entry<Entity, Vector4f> entry : positions.entrySet()) {
            ...
            if (entry.getKey() instanceof LivingEntity entity) {
                ...
                if (!remove.getValueByName("Боксы").get()) {
                    DisplayUtils.drawRoundedBox(position.x - 0.5f, position.y - 0.5f, position.z + 0.5f, position.w + 0.5f, 2, 50, ColorUtils.rgba(0, 0, 0, 128));
                ...
                if (!remove.getValueByName("Полоску хп").get()) {
                    ...
                    DisplayUtils.drawRoundedBox(position.x, position.y, position.z, position.w, 10, 100, ColorUtils.rgba(0, 0, 0, 128));
